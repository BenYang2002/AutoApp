...
import { applyToJob } from "./Linkedin-apply.js";
import { hasAppliedToJob } from "../db/linkedin.js";
...
export async function navigateThroughJobs(page: Page) {
  while (true) {
    ...
    for (let i = 0; i < jobCount; i++) {
      ...
      if (await hasAppliedToJob(jdId)) {
        console.log(`Already applied to job ${jdId}, skipping`);
        continue;
      }
      ...
      await applyToJob(page, jdId, jdMarkdown, result);
